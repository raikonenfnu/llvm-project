...
IntLoadOpPattern::matchAndRewrite(memref::LoadOp loadOp, OpAdaptor adaptor,
  ...
  if(!(isScalar ^ isComposite)) return rewriter.notifyMatchFailure(loadOp, "Designated pointer type needs to be scalar or composite.");
  ...
  if(isComposite) {
    accessChainOp =
        spirv::getElementPtr(typeConverter, memrefType, adaptor.memref(),
                            adaptor.indices(), loc, rewriter).getOperation();
  } else {
    accessChainOp =
        spirv::getElementPtrDirect(typeConverter, memrefType, adaptor.memref(),
                            adaptor.indices(), loc, rewriter).getOperation();
  }
  if (!accessChainOp)
    return failure();
  ...
  if(isComposite) {
    ...
    assert(dyn_cast<spirv::PtrAccessChainOp>(accessChainOp).indices().size() == 2);
  }

  Value adjustedPtr;
  if(isComposite) {
    adjustedPtr = adjustAccessChainForBitwidth(typeConverter, dyn_cast<spirv::AccessChainOp>(accessChainOp),
                                                    srcBits, dstBits, rewriter);
  } else {
    adjustedPtr = adjustAccessChainForBitwidth(typeConverter, dyn_cast<spirv::PtrAccessChainOp>(accessChainOp),
                                                    srcBits, dstBits, rewriter);
  }
...
IntStoreOpPattern::matchAndRewrite(memref::StoreOp storeOp, OpAdaptor adaptor,
  ...
  bool isScalar = spirv::ScalarType::classof(pointeeType);
  bool isComposite = spirv::CompositeType::classof(pointeeType);
  if(!(isScalar ^ isComposite)) return rewriter.notifyMatchFailure(storeOp, "Designated pointer type needs to be scalar or composite.");
  Type dstType = pointeeType;
  if(isComposite) {
    Type structElemType = pointeeType.cast<spirv::StructType>().getElementType(0);
    if (auto arrayType = structElemType.dyn_cast<spirv::ArrayType>())
