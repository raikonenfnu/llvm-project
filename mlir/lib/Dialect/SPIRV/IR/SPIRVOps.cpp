...
static Type getElementPtrType(Type type, ValueRange indices, Location baseLoc) {
  auto ptrType = type.dyn_cast<spirv::PointerType>();
  if (!ptrType) {
    llvm::outs()<<"spv.AccessChain' op expected a pointer to composite type, but provided\n";
  ...
  for (auto indexSSA : indices) {
    ...
    if (!cType) {
      llvm::outs()<<"spv.AccessChain' op cannot extract from non-composite type\n";
      emitError(baseLoc,
    ...
    if (resultType.isa<spirv::StructType>()) {
      ...
      if (!op) {
        llvm::outs()<<"spv.AccessChain' op index must be an integer spv.Constant to access element of spv.struct\n";
      ...
      if (failed(extractValueFromConstOp(op, index))) {
        llvm::outs()<<"spv.AccessChain' index must be an integer spv.Constant to access element of spv.struct, but provided\n";
        emitError(baseLoc,
...
void spirv::AccessChainOp::build(OpBuilder &builder, OperationState &state,
  ...
  if(type == nullptr) llvm::outs()<<"bad ptr:"<<basePtr<<"\n";
  assert(type && "Unable to deduce return type based on basePtr and indices for good old AccessChain");
...
void spirv::PtrAccessChainOp::build(OpBuilder &builder, OperationState &state,
                                    ...
                                    ValueRange indices) {
  auto type = getElementPtrType(basePtr.getType(), indices, state.location);
  assert(type && "Unable to deduce return type based on basePtr and indices for PtrAccessChain");
  build(builder, state, type, basePtr, element, indices);
}
